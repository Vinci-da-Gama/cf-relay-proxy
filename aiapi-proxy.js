  ...
  async handle(req) {
    try {
      ...
      if (!data.candidates || data.candidates.length === 0) {
        return new Response(JSON.stringify({ error: "No candidates returned from API", details: data.promptFeedback || data }), { status: 200 });
      }
      ...
      return new Response(JSON.stringify({ error: "Internal processing error", details: e.message }), { status: 500 });
    }
  }
